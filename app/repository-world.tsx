...
export type RepositoryWorldProject = {
  ...
  activityKnown?:boolean; private?:boolean; stars?:number; topics?:string[]; lastTouched?:string;
};
...
type RepositoryWorldProps = {
  ...
  kicker?:string;
  showTours?:boolean;
  year?:number; throughMonth?:number;
};

export default function RepositoryWorld({projects,heading="The repository world",description="A living map of the projects. Proximity comes from shared ideas, language, project families, and purpose; stronger relationships pull repositories closer together.",kicker,showTours=true,year=2026,throughMonth=12}:RepositoryWorldProps) {
  const monthCount = Math.max(1, Math.min(12, throughMonth));
  ...
  useEffect(()=>{
    ...
    const draw=()=>{
      ...
      namedNodes.sort((a,b)=>Number(b.name===nodes[hover]?.name)-Number(a.name===nodes[hover]?.name));
      namedNodes.forEach(node=>{
        const point=projectPoint(node),radius=nodeRadius(node),highlighted=node===nodes[hover]||node===nodes[drag]||node===nodes[selectedIndex];
        if(!inTour(node)&&!highlighted)return;
        context.font="14px ui-monospace, monospace";context.textAlign="center";
        const labelWidth=context.measureText(node.title).width;
        const x=Math.max(labelWidth/2+8,Math.min(width-labelWidth/2-8,point.x));
        const positions=[point.y-radius-14,point.y+radius+24];
        const y=positions.find(baseline=>{
          const box={left:x-labelWidth/2-4,right:x+labelWidth/2+4,top:baseline-15,bottom:baseline+4};
          if(box.top<4||box.bottom>height-36)return false;
          if(labels.some(label=>box.left<label.right&&box.right>label.left&&box.top<label.bottom&&box.bottom>label.top))return false;
          return !nodes.some(other=>{if(other===node||!visible(other))return false;const p=projectPoint(other),r=nodeRadius(other)+3;return p.x+r>box.left&&p.x-r<box.right&&p.y+r>box.top&&p.y-r<box.bottom;});
        })??(highlighted?positions[0]:undefined);
        if(y===undefined)return;
        context.save();context.lineWidth=5;context.strokeStyle="#05121d";context.strokeText(node.title,x,y);context.fillStyle="#e1f7ff";context.fillText(node.title,x,y);context.restore();
        labels.push({left:x-labelWidth/2-4,right:x+labelWidth/2+4,top:y-15,bottom:y+4});
      });
    };
    ...
    const leave=()=>{hover=-1;if(reduceMotion)draw();};
    ...
    observer.observe(canvas);visibility.observe(canvas);resize();animate();canvas.addEventListener("pointermove",move);canvas.addEventListener("pointerdown",down);canvas.addEventListener("pointerup",up);canvas.addEventListener("pointercancel",cancel);canvas.addEventListener("pointerleave",leave);
    return()=>{cancelAnimationFrame(frame);observer.disconnect();visibility.disconnect();canvas.removeEventListener("pointermove",move);canvas.removeEventListener("pointerdown",down);canvas.removeEventListener("pointerup",up);canvas.removeEventListener("pointercancel",cancel);canvas.removeEventListener("pointerleave",leave);};
  ...
  return <section className="repositoryWorld section" id="world">
    ...
    <div className="worldTimeline">
      <button type="button" className="timelinePlay" aria-label={playing?"Pause repository timeline":"Play repository timeline"} onClick={()=>{if(playing){setPlaying(false);return;}if(month===monthCount)setMonth(1);setPlaying(true);}}>{playing?"Ⅱ":"▶"}</button>
      <label><span>{year} / <b>{monthNames[month-1]}</b></span><input type="range" min="1" max={monthCount} step="1" value={month} aria-label={`Repository world through ${monthNames[month-1]} ${year}`} onChange={event=>{setPlaying(false);setMonth(Number(event.target.value));}}/><i style={{width:`${(month-1)/Math.max(1,monthCount-1)*100}%`}}/></label>
      <div className="timelineMonths" aria-hidden="true">{monthNames.slice(0,monthCount).map((name,index)=><span className={index+1<=month?"reached":""} key={name}>{name}</span>)}</div>
